...
dishRouter
  ...
  .post(cors.corsWithOptions, cors.corsWithOptions, authenticate.verifyToken, authenticate.verifyAdmin, (req, res, next) => {
    res.statusCode = 403;
...
dishRouter
      ...
      .then(
        (dish) => {
          if (dish !== null && dish.comments !== null) {
            ...
            let err = new Error("Comment " + req.params.dishId + " not found");
            err.status = 404;
            next(err);
          }
        },
  ...
  .put(cors.corsWithOptions, authenticate.verifyToken, (req, res, next) => {
    Dishes.findById(req.params.dishId)
      .then(
        (dish) => {
          let currentComment = dish.comments.id(req.params.commentId)
          if(!currentComment.author.equals(req.user.id)) {
            let err = new Error("Only commentator can perform this operation!")
            err.status = 403
            next(err)
          }else if (dish !== null && dish.comments !== null) {
            if (req.body.rating) {
